...
function formArray<T extends FormValue>(initialValue: T[]) {
  ...
  const set = (value: T[]) => {
    if (!Array.isArray(value)) {
      throw new Error(`Cannot set a form with fields to a primitive value (${value})`);
    }
...
export function form<T extends FormValue>(initialValue: T): Form<T> {
  ...
  const {
    value, set, fields
  } = Array.isArray(initialValue) ? formArray(initialValue) : formRecord(initialValue);
